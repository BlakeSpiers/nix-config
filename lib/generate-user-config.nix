...
let
  fn = { flake, config, pkgs, user-settings, modules, overrides ? { }, ... }:
    ...
    let
      ...
      localDomain =
        if hasAttr "localDomain" config.networking then
          config.networking.localDomain
        else
          config.networking.domain;

      darwinHosts =
        if hasAttr "darwinConfigurations" flake then
      ...
      linuxHosts =
        if builtins.hasAttr "nixosConfigurations" flake then
          (map (x: x.config.networking.hostName)
